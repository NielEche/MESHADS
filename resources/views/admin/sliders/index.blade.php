			...
			<div class=" container-fluid   container-fixed-lg sm-p-l-0 sm-p-r-0">
				<div class="inner">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item bold active">Home Sliders</li>
					</ol>
				</div>
			</div>
		</div>
			...
			<div class="col-md-12">
				<div class="panel panel-transparent">
					...
					<span class="p-l-10 p-r-10 p-b-10 p-t-10 m-r-10">
						...
						<button type="button" data-toggle="modal" data-target="#createModal" class="btn btn-default btn-sm m-r-10"><i class="pg-plus"></i>Add New Slider</button>
				...
				{{-- Begin Preview Header Modal --}}
					@foreach ($sliders as $slider)
						<div class="modal fade fill-in" id="previewModal{{ $slider->id }}" tabindex="-1" role="dialog" aria-labelledby="previewModalLabel{{ $slider->id }}" aria-hidden="true">
							<button class="close" data-dismiss="modal" aria-label="close">
								<span aria-hidden="true"><i class="fa fa-times"></i></span>
							</button>
				...
				{{-- Begin Delete Header Modal --}}
					@foreach ($sliders as $slider)
						<div class="modal fade slide-down disable-scroll" id="deleteModal{{ $slider->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel{{ $slider->id }}" aria-hidden="true">

							<div class="modal-dialog modal-sm">
 
								<div class="modal-content">
						<form id="mesh{{$slider->id}}" action="{{ route('admin.sliders.destroy', $slider->id) }}" method="POST" >
								@csrf
								@method('DELETE')									
										<div class="modal-header">
											<button class="close" data-dismiss="modal" aria-label="close">
				...
				{{-- Create New Modal --}}
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content-wrapper">
								<div class="modal-content">
									...
									<div class="modal-body">
										<form method="POST" role="form" action="{{ route('admin.sliders.store') }}" enctype="multipart/form-data">
							...
							<div class="form-group form-group-default required @error('type') has-error @enderror">
							...
							<div class="form-group form-group-default @error('link_url') has-error @enderror">
								...
								<input type="text" class="form-control @error('link_url') is-invalid @enderror" name="link_url" value="{{ old('link_url') }}">
								@error('link_url')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="form-group form-group-default @error('link_text') has-error @enderror">
								<label>Video URL</label>
								<input type="text" class="form-control @error('link_text') is-invalid @enderror" name="link_text" value="{{ old('link_text') }}">
								@error('link_text')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="form-group form-group-default required @error('order_number') has-error @enderror">
								<label>Order number</label>
								<input type="text" class="form-control @error('order_number') is-invalid @enderror" name="order_number" value="{{ old('order_number') }}" required>
					...
					{{-- Edit Modal --}}
			@foreach ($sliders as $slider)
					<div class="modal fade slide-down disable-scroll" id="editModal{{ $slider->id }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $slider->id }}" aria-hidden="true">
						<div class="modal-dialog modal-lg">
							<div class="modal-content-wrapper">
								<div class="modal-content">
									<div class="modal-header clearfix text-left">
										...
										<h5 class="bold">Edit Slider</h5>
									</div>
									<div class="modal-body">
						<form method="POST" role="form" action="{{ route('admin.sliders.update', $slider->id) }}" enctype="multipart/form-data">
							...
							<div class="form-group form-group-default @error('title') has-error @enderror">
								<label>Title</label>
								<input type="text" class="form-control @error('title') is-invalid @enderror" name="title" value="{{ old('title') ?? $slider->title }}">
								@error('title')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="form-group form-group-default @error('caption') has-error @enderror">
								<label>Caption</label>
								<input type="text" class="form-control @error('caption') is-invalid @enderror" name="caption" value="{{ old('caption') ?? $slider->caption }}">
								@error('caption')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="form-group form-group-default required @error('type') has-error @enderror">
								...
								<select class="form-control @error('type') is-invalid @enderror" name="type">
									<option value="">Select Type</option>
									<option value="image" {{ $slider->type == 'image' ?? old('type') == 'image'  ? 'selected' : '' }}>Image</option>
									<option value="video" {{ $slider->type == 'video' ?? old('type') == 'video' ? 'selected' : '' }}>Video</option>
								</select>
								@error('type')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
							...
							<div class="form-group form-group-default @error('link_url') has-error @enderror">
								<label>Link on slider</label>
								<input type="text" class="form-control @error('link_url') is-invalid @enderror" name="link_url" value="{{ old('link_url') ?? $slider->link_url }}">
								@error('link_url')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="form-group form-group-default @error('link_text') has-error @enderror">
								...
								<input type="text" class="form-control @error('link_text') is-invalid @enderror" name="link_text" value="{{ old('link_text') ?? $slider->link_text }}">
								@error('link_text')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="form-group form-group-default required @error('order_number') has-error @enderror">
	                            <label>Order number</label>
								<input type="text" class="form-control @error('order_number') is-invalid @enderror" name="order_number" value="{{ old('order_number') ?? $slider->order_number }}">
							...
							<div class="pull-left">
								<div class="checkbox check-success  ">
									<input type="checkbox" {{ $slider->is_visible ?? old('is_visible') ? 'checked' : '' }} name="is_visible" id="checkbox-agree">
					...
					<div class="col-lg-3 item">
						<div class="card card-default">
							<div class="card-header ">
								<input type="checkbox" name="checkbox[]" value="{{ $slider->id }}" class="table_checkbox">&nbsp;
								<div class="card-title">{{ $slider->title }}	</div>
						<div class="card-controls">
							<ul>

								<li><a class="card-refresh" href="#" data-toggle="modal" data-target="#editModal{{ $slider->id }}"><i class="fa fa-pencil"></i></a>
								</li>
								<li><a class="card-close" href="#" data-toggle="modal" data-target="#deleteModal{{ $slider->id }}"><i class="fa fa-times"></i></a>
								</li>
								</ul>
						</div>
							</div>
