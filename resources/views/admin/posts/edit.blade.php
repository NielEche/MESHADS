...
<div class="content">
	...
	<div class=" container-fluid container-fixed-lg bg-white p-t-20 p-b-20">
			...
			<div class="col-lg-12">
				...
				<div class="card card-transparent">
					<div class="card-header m-b-10">
						<div class="card-title"><h6>Edit {{ $post->title }}</h6></div>
					</div>
					<div class="card-body">

						<form method="POST" role="form" action="{{ route('admin.posts.update', $post->id) }}" enctype="multipart/form-data">
							...
							<div class="form-group form-group-default @error('title') has-error @enderror">
								<label>Caption</label>
								<input type="text" class="form-control @error('caption') is-invalid @enderror" name="caption" value="{{ old('caption') ?? $post->caption }}" required>
								@error('caption')
	                                <span class="invalid-feedback" role="alert">
	                                    {{ $message }}
	                                </span>
							...
							<div class="summernote-wrapper m-t-20 @error('top_content') has-error @enderror">
								<span style="text-transform: uppercase;">Top Content <span class="text-danger" style="font-size: 17px;">*</span></span> <br>
								<textarea name="top_content" class="summernote form-control @error('top_content') is-invalid @enderror" 
								rows="19">{{ old('top_content') ?? $post->top_content }}</textarea>
							</div>
							@error('top_content')
                                <span class="invalid-feedback" role="alert">
                                    {{ $message }}
							...
							<div class="form-group form-group-default">
								<label>Quote</label>
								<textarea class="form-control" name="quote" rows="10" style="height:100px;">{{ old('quote') ?? $post->quote  }}</textarea>
							</div>
								
							<div class="form-group form-group-default">
								<label>Quote Author</label>
								<input type="text" class="form-control" name="quote_author" value="{{ old('quote_author') ?? $post->quote_author }}">
							</div>

							<div class="summernote-wrapper m-t-20 @error('middle_content') has-error @enderror">
								<span style="text-transform: uppercase;">Middle Content <span class="text-danger" style="font-size: 17px;">*</span></span> <br>
								<textarea name="middle_content" class="summernote form-control @error('middle_content') is-invalid @enderror" 
								rows="19">{{ old('middle_content') ?? $post->middle_content }}</textarea>
							</div>
							...
							<div class="summernote-wrapper m-t-20 @error('bottom_content') has-error @enderror">
								<span style="text-transform: uppercase;">Bottom Content <span class="text-danger" style="font-size: 17px;">*</span></span> <br>
								<textarea name="bottom_content" class="summernote form-control @error('bottom_content') is-invalid @enderror" 
								rows="19">{{ old('bottom_content') ?? $post->bottom_content }}</textarea>
							</div>
							...
							<div class="form-group form-group-default">
								...
								<input type="text" class="form-control" name="video_url" value="{{ old('video_url') ?? $post->video_url }}">
							</div>
							...
							<div class="pull-left">
								<div class="checkbox check-success  ">
									<input type="checkbox" {{ old('is_published') ?? $post->is_published ? 'checked' : '' }} name="is_published" id="checkbox-agree">
